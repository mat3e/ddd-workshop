...
import io.github.mat3e.tasker.task.TaskQueryRepository;
...
public class ProjectFacade {
    ...
    ProjectFacade(final ProjectRepository projectRepository, final TaskFacade taskFacade, final TaskQueryRepository taskQueryRepository) {
    ...
    List<TaskDto> createTasks(int projectId, ZonedDateTime projectDeadline) {
        if (taskQueryRepository.areUndoneTasksWithProjectId(projectId)) {
        ...
        return get(projectId).map(project -> {
            List<TaskDto> tasks = project.getSteps()
                    .stream()
                    .map(step -> TaskDto.of(step.getDescription(), projectDeadline.plusDays(step.getDaysToProjectDeadline())))
                    .collect(toList());
